<div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-y-6 gap-x-14">
    @forelse ($books as $book)
        <div
            ...
            <!-- Image -->
            <img class="object-cover w-full transition-all duration-300 ease-in-out h-80 group-hover:h-full"
                src="{{ $book->image }}" alt="{{ $book->name }}">
            <div
                class="absolute inset-0 flex flex-col justify-center p-5 py-5 transition-opacity duration-300 bg-white opacity-0 bg-opacity-90 group-hover:opacity-100">
