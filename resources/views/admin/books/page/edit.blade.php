        ...
        <form method="POST" action="{{ route('admin.page.update', $page) }}" enctype="multipart/form-data">
            ...
            <div class="grid gap-5 md:grid-cols-2">
                ...
                <div>
                    ...
                    <select id="book" name="book_id" class="block w-full mt-1">
                        @foreach ($books as $book)
                            <option value="{{ $book->id }}" {{ $page->book_id == $book->id ? 'selected' : '' }}>
                                {{ $book->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <x-input id="body" class="block w-full mt-1" type="text" name="body"
                        value="{{ $page->body }}" required autofocus autocomplete="body" />
                    {{-- <x-input-error :messages="$errors->get('name')" class="mt-2" /> --}}
                </div>
            </div>
